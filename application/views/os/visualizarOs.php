...
<div class="row-fluid" style="margin-top: 0">
    <div class="span12">
        <div class="widget-box">
            <div class="widget-title" style="margin: 10px 0 0">
                <div class="buttons">
                    ...
                    <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vOs')) {
                        ...
                        $texto_de_notificacao = $this->os_model->criarTextoWhats('', $troca);
                        if (!empty($zapnumber)) {
            ...
            <?php $abaAtiva = $abaAtiva ?? 'os'; ?>

            <ul class="nav nav-tabs" style="margin-bottom:15px">
                <li class="<?= ($abaAtiva === 'os') ? 'active' : '' ?>">
                    <a href="#tab-os" data-toggle="tab">
                        <i class="bx bx-file"></i> OS
                    </a>
                </li>

                <li class="<?= ($abaAtiva === 'producao') ? 'active' : '' ?>">
            ...
            <div class="tab-content">
                ...
                <!-- ==========================
                     ABA OS
                ========================== -->
                <div class="tab-pane <?= ($abaAtiva === 'os') ? 'active' : '' ?>" id="tab-os">
                                            ...
                                            <td style="width: 40%; padding-left: 0">
                                                <ul>
                                                    <li>
                                                        ...
                                                        <span><b><i class="fas fa-user"></i> <?php echo $result->nome ?></b></span><br />
                                                        <span><i class="fas fa-phone"></i> <?php echo $result->telefone_usuario ?></span><br />
                                                        <span><i class="fas fa-envelope"></i> <?php echo $result->email_usuario ?></span>
                                                    </li>
                                                </ul>
                                            </td>
                                        </tr>
                ...
                <!-- ==========================
                     ABA PRODUÇÃO (SEM DUPLICAR)
                ========================== -->
                <div class="tab-pane <?= ($abaAtiva === 'producao') ? 'active' : '' ?>" id="tab-producao">

                    <!-- ABAS INTERNAS: Produção 1 / 2 / 3 + -->
                    <ul class="nav nav-tabs" style="margin-bottom:15px">
                        <?php if (!empty($producoes)): ?>
                            <?php foreach ($producoes as $i => $p): ?>
                                <li class="<?= (!empty($producaoAtual) && $p->id == $producaoAtual->id) ? 'active' : '' ?>">
                                    <a href="<?= site_url('os/visualizar/' . $result->idOs . '?tab=producao&producao=' . $p->id) ?>">
                                        <i class="bx bx-layer"></i> Produção <?= $i + 1 ?>
                                    </a>
                                </li>
                            <?php endforeach; ?>
                        <?php endif; ?>
                        ...
                        <li>
                            <a href="<?= site_url('os/novaProducao/' . $result->idOs) ?>" title="Adicionar nova produção">
                    ...
                    <div class="widget-content">
                        <form action="<?= site_url('os/salvarProducao') ?>"
                            ...
                            <input type="hidden" name="producao_id" value="<?= $producaoAtual->id ?? '' ?>">
...
<div class="form-group">
    <label>
        <input type="checkbox" id="chkPrioridade">
        Prioridade
    </label>
</div>

<div class="form-group" id="boxPrioridade" style="display:none;">
    <label>Tipo de Prioridade</label>
    <select name="prioridade" class="form-control">
        <option value="">Selecione</option>
        <option value="URGENTE">URGENTE!</option>
        <option value="EVENTO">EVENTO – NÃO PODE TER ATRASO</option>
        <option value="RETRABALHO">RETRABALHO</option>
    </select>
</div>

<script>
document.getElementById('chkPrioridade').addEventListener('change', function () {
    document.getElementById('boxPrioridade').style.display =
        this.checked ? 'block' : 'none';
                            ...
                            <fieldset>
                                ...
                                <textarea name="observacao" class="span12" rows="3"><?= $producaoAtual->observacao ?? '' ?></textarea>
                            </fieldset>
                            ...
                            <!-- WHATSAPP -> CONVERTER -->
                            <fieldset>
                                <legend><i class="bx bxl-whatsapp"></i> Colar grade recebida do WhatsApp</legend>
                                <textarea id="gradeWhatsapp" class="span12" rows="4"
                                          placeholder="Cole aqui a mensagem do WhatsApp no formato:
                                QTD-NOME-SUPERIOR-INFERIOR-N°-ADICIONAL-MODELO"></textarea>

                                <button type="button" id="btnConverterWhatsapp" class="btn btn-mini btn-success" style="margin-top:8px;">
                                    <i class="bx bx-transfer"></i> Converter para grade
                                </button>
                            </fieldset>
                            ...
                            <fieldset>
                                ...
                                <div style="margin-bottom:10px;">
                                    <input type="number" id="qtdLinhas" value="1" style="width:70px;">
                                    <button type="button" class="btn btn-mini btn-info" id="btnAddLinhas">
                                        + Adicionar linhas
                                    </button>
                                </div>
                                ...
                                <table class="table table-bordered table-condensed" id="tabela-grade">
                                    ...
                                    <tbody>
                                        <?php
                                        $linhas = !empty($producaoGrade) ? $producaoGrade : [[]];

                                        foreach ($linhas as $i => $linha):
                                            // aceita tanto array quanto objeto
                                            $get = function($k) use ($linha) {
                                                if (is_array($linha)) return $linha[$k] ?? '';
                                                if (is_object($linha)) return $linha->$k ?? '';
                                                return '';
                                            };
                                        ?>
                                            <tr>
                                                <td><input type="number" name="grade[<?= $i ?>][quantidade]" class="span12" value="<?= $get('quantidade') ?>"></td>
                                                <td><input type="text" name="grade[<?= $i ?>][nome]" class="span12" value="<?= $get('nome') ?>"></td>
                                                <td><input type="text" name="grade[<?= $i ?>][superior]" class="span12" value="<?= $get('superior') ?>"></td>
                                                <td><input type="text" name="grade[<?= $i ?>][inferior]" class="span12" value="<?= $get('inferior') ?>"></td>
                                                <td><input type="text" name="grade[<?= $i ?>][numero]" class="span12" value="<?= $get('numero') ?>"></td>
                                                <td><input type="text" name="grade[<?= $i ?>][adicional]" class="span12" value="<?= $get('adicional') ?>"></td>
                                                <td>
                                                    <select name="grade[<?= $i ?>][modelo]" class="span12">
                                                        <?php foreach (['UNISSEX','FEM','MASC','INFANTIL'] as $m): ?>
                                                            <option value="<?= $m ?>" <?= strtoupper($get('modelo')) === $m ? 'selected' : '' ?>><?= $m ?></option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                </td>
                                                <td>
                                                    <button type="button" class="btn btn-danger btn-mini remover-linha">X</button>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                            ...
                            <hr>
                            <a href="<?= site_url('os/exportarGradeExcel/' . $producaoAtual->id) ?>"
...
$('#copyButton').on('click', function() {
    ...
    if (code) {
        navigator.clipboard.writeText(code.data).then(function() {
            ...
            swal({ type: "success", title: "Sucesso!", text: "QR Code copiado com sucesso: " + code.data, icon: "success", timer: 3000, showConfirmButton: false });
        }).catch(function(err) {
            swal({ type: "error", title: "Atenção", text: "Erro ao copiar QR Code: ", err });
        });
    } else {
...
<script type="text/javascript">
/**
 * ====== GRADE: adicionar/remover/whatsapp ======
 * Guardas para não quebrar a página se algum elemento não existir.
 */
...
const btnAddLinhas = document.getElementById('btnAddLinhas');
if (btnAddLinhas) {
    btnAddLinhas.addEventListener('click', function () {
...
document.addEventListener('click', function (e) {
    if (e.target.classList && e.target.classList.contains('remover-linha')) {
        e.target.closest('tr').remove();
    }
});

const btnConverterWhatsapp = document.getElementById('btnConverterWhatsapp');
if (btnConverterWhatsapp) {
    btnConverterWhatsapp.addEventListener('click', function () {
